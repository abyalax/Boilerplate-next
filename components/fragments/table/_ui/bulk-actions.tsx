 ...
 * @param {React.ReactNode} props.bulkActions The action buttons to be rendered inside the toolbar.
...
export const BulkActions = <TData,>({ table, bulkActions }: Props<TData>): ReactNode | null => {
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message = `${selectedCount} ${selectedCount > 1 ? 's' : ''} selected. Bulk actions toolbar is available.`;
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role="toolbar"
        aria-label={`Bulk actions for ${selectedCount} selected ${selectedCount > 1 ? 's' : ''}`}
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-x-1 text-sm" id="bulk-actions-description">
            ...
            <span className="hidden sm:inline">{selectedCount > 1 ? 's' : ''}</span> selected
          </div>
